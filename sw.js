...
importScripts('https://palmbeachtech.org/wp-content/plugins/chrome-push-notifications/assets/js/library/Dexie.min.js');
...
self.addEventListener('push', function(event) {
  ...
  event.waitUntil(self.registration.pushManager.getSubscription().then(function(subscription) {
   if (_webPushConfig.debug) console.log('Push sub data', subscription);
      return fetch('http://palmbeachtech.org/?subId='+ encodeURIComponent(subscription.endpoint)).then(function(response) {  
...
function showNotification(title, body, image, tag) {
    var options = {
        body: body,
        tag: tag,
        icon: 'https://palmbeachtech.org/wp-content/uploads/2016/11/PBT-Social-1-resized.png'
    };
